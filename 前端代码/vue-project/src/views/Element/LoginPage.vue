  ...
  <div v-loading="loading" element-loading-text="登录中..." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.6)" class="login-container">
    ...
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <div class="title-container">
        <h3 class="title">PowerGame-show your power</h3>
      </div>
      ...
      <div>
        <el-button type="primary" style="width:100%;margin-bottom:20px;" @click.native.prevent="handleLogin">
            <span style="text-decoration: none;">登录</span>
        </el-button>
      </div>
      <div>
      ...
      <div class="tips">
        <el-button type="primary" style="width:100%;margin-bottom:20px;" @click.native.prevent="handleStrangerLogin">
          游客登录
        </el-button>
      </div>
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate((valid) => {
        ...
        try {              
          axios.post('/login', {//用post方法传 输入框输入的用户名和密码
                      ...
                      .then((successResponse) => {//回调函数当post成功后执行
                        ...
                        if (successResponse.data.code === 1) {//如果后端返回的状态码是1，则进入用户页面
                          if (valid) {
                            this.loading = true
                            setTimeout(() => {
                        ...
                        if (successResponse.data.code === 2) {//如果后端返回的状态码是2，则进入管理员页面
                          if (valid) {
                            this.loading = true
                            setTimeout(() => {
                        ...
                        if (successResponse.data.code === 3) {//如果后端返回的状态码是3，则进入超级管理员页面
                          if (valid) {
                            this.loading = true
                            setTimeout(() => {
    ...
    handleStrangerLogin() {
      axios.post('/login', {//用post方法传 输入框输入的用户名和密码
                      ...
                      this.$store.dispatch('setUsername',"strange")
    },
    // 注册业务
    handleRegister() {
